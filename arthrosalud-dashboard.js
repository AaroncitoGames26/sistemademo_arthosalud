...
const MENU_CAJA = [
  ...
  { nombre:'Consumo', items:[
    {t:'Pago'}, {t:'Registro Y Pago'}, {t:'Nota De Crédito'}, {t:'Venta Sin Admisión'}
  ]},
  { nombre:'Ficheros', items:[
...
function renderResumenCaja(m){
  const abierta = CAJA_ACTUAL.abierta;
  const bannerEstado = abierta
    ? `<div class="caja-estado-turno abierta"><span class="dot on"></span><div><strong>Caja abierta: ${CAJA_ACTUAL.caja}</strong><small>Turno ${CAJA_ACTUAL.fecha} · ${CAJA_ACTUAL.usuario} · Importe S/ ${CAJA_ACTUAL.monto.toFixed(2)}</small></div></div>`
    : `<div class="caja-estado-turno cerrada"><span class="dot off"></span><div><strong>No hay caja abierta</strong><small>Para registrar pagos y consumos debes aperturar una caja primero.</small></div><button class="btn-primario" data-volver-caja data-ir-apertura>＋ Aperturar caja</button></div>`;
  const accesos = abierta
    ? `<button class="admision-acceso" data-seccion="Pago">Registrar pago</button><button class="admision-acceso" data-seccion="Registro Y Pago">Registro y pago</button><button class="admision-acceso" data-seccion="Nota De Crédito">Nota de crédito</button><button class="admision-acceso" data-seccion="Cierre">Cerrar caja</button>`
    : `<button class="admision-acceso" data-seccion="Apertura">Aperturar caja</button><button class="admision-acceso" data-seccion="Registros">Historial de cajas</button>`;
  return `<div class="admision-resumen"><div class="migaja">Módulo <b>${m.nombre}</b></div><h3>Resumen de Caja</h3><p class="admision-resumen-intro">Control de pagos, comprobantes y operaciones del turno.</p>${bannerEstado}<div class="admision-kpis"><div class="admision-kpi"><strong>${formatoMoneda(totalIngresosCaja())}</strong><span>Ingresos del día</span></div><div class="admision-kpi"><strong>${MOVIMIENTOS_CAJA.filter(x=>x.estado!=='Anulado' && String(x.concepto).indexOf('Nota Credito')!==0).length}</strong><span>Transacciones</span></div><div class="admision-kpi"><strong>${formatoMoneda(totalPendienteCobro())}</strong><span>Pendiente de cobro</span></div><div class="admision-kpi"><strong>${REGISTROS_CAJAS.filter(r=>r.estado==='ABIERTO').length}</strong><span>Cajas abiertas</span></div></div><div class="admision-columnas"><section class="admision-bloque"><div class="admision-bloque-header"><h4>Últimos movimientos</h4><span>Hoy</span></div>${MOVIMIENTOS_CAJA.slice(-3).reverse().map(x=>`<div class="admision-fila"><span class="admision-hora">${x.fecha.split(' ')[1]||''}</span><div><b>${x.paciente}</b><small>${x.concepto} · ${x.comprobante}</small></div><span class="admision-estado">${x.estado}</span></div>`).join('')}</section><section class="admision-bloque"><div class="admision-bloque-header"><h4>Requiere atención</h4><span>${MOVIMIENTOS_CAJA.filter(x=>x.estado==='Pendiente').length} pendientes</span></div>${MOVIMIENTOS_CAJA.filter(x=>x.estado==='Pendiente').slice(0,2).map(x=>`<div class="admision-alerta"><span class="admision-alerta-punto critico"></span><div><strong>Cobro pendiente</strong><small>${x.paciente} · ${x.concepto} · ${formatoMoneda(x.monto)}</small></div></div>`).join('')}<div class="admision-alerta"><span class="admision-alerta-punto"></span><div><strong>Cierre de caja próximo</strong><small>Caja principal · faltan 45 minutos</small></div></div></section></div><div class="admision-accesos">${accesos}</div></div>`;
}
...
function activarMenuInterno(){
  ...
  document.querySelectorAll('#navbar-interna .submenu a').forEach(a=>{
    a.addEventListener('click', (e)=>{
      ...
      if(menuPadre === 'Consumo' && seccion === 'Pago'){
...
document.addEventListener('click', (e)=>{
  const cajaMod = MODULOS.find(x=>x.id==='caja');
  const acceso = e.target.closest('.admision-acceso');
  if(acceso && e.target.closest('.panel-detalle')){
    const opcion = document.querySelector(`#navbar-interna a[data-seccion="${acceso.dataset.seccion}"]`);
    if(opcion) opcion.click();
    return;
  }
  if(!cajaMod) return;
  const volver = e.target.closest('[data-volver-caja]');
  if(volver && e.target.closest('.panel-detalle')){
    const panel = document.querySelector('.panel-header h2');
    if(panel && panel.textContent.includes(cajaMod.nombre)){
      const irApertura = volver.hasAttribute('data-ir-apertura');
      if(irApertura){
        renderCajaSeccion('Apertura', 'Caja');
        return;
      }
      const vista = document.getElementById('vista-seccion');
      vista.innerHTML = renderResumenCaja(cajaMod);
      return;
    }
  }
});
...
function activarPagoConsumo(){
  ...
  form.addEventListener('submit', function(e){
    ...
    form.insertAdjacentHTML('beforebegin','<div class="caja-confirmacion"><span>✓</span><div><strong>Pago registrado ' + nroComprobante + '</strong><span>' + nuevoMovimiento.paciente + ' · ' + nuevoMovimiento.concepto + ' · ' + formatoMoneda(nuevoMovimiento.monto) + ' · ' + nuevoMovimiento.medioPago + '</span></div></div>');
    form.reset();
    inputDNI.value = '';
...
function renderRegistroYPago(menuPadre){
  ...
  if(!CAJA_ACTUAL.abierta){
    vista.innerHTML = '<div class="caja-vista"><div class="migaja">Caja <b>/ Consumo</b></div><div class="caja-volver"><button data-volver-caja>← Volver al resumen de Caja</button></div><div class="caja-form-panel caja-vacio"><div class="caja-vacio-icono">⚠</div><h4>No hay caja abierta</h4><p>Apertura una caja primero para registrar pagos.</p><button class="btn-primario" id="ir-apertura-ryp">Ir a Apertura</button></div></div>';
  ...
  form.addEventListener('submit', function(e){
    ...
    var formRYP = document.getElementById('form-ryp');
    formRYP.insertAdjacentHTML('beforebegin','<div class="caja-confirmacion"><span>✓</span><div><strong>Servicio registrado y cobrado ' + nroComprobante + '</strong><span>' + nuevo.paciente + ' · ' + nuevo.concepto + ' · ' + formatoMoneda(nuevo.monto) + '</span></div></div>');
...
function renderVentaSinAdmision(menuPadre){
  ...
  form.addEventListener('submit', function(e){
    ...
    MOVIMIENTOS_CAJA.push(nuevo);
    var formVSA = document.getElementById('form-vsa');
    formVSA.insertAdjacentHTML('beforebegin','<div class="caja-confirmacion"><span>✓</span><div><strong>Venta sin admision emitida ' + nroComprobante + '</strong><span>' + nuevo.paciente + ' · ' + nuevo.concepto + ' · ' + formatoMoneda(total) + ' · ' + nuevo.medioPago + '</span></div></div>');
...
function renderNotaCredito(menuPadre){
  var vista = document.getElementById('vista-seccion');
  if(!CAJA_ACTUAL.abierta){
    vista.innerHTML = '<div class="caja-vista"><div class="migaja">Caja <b>/ Consumo</b></div><div class="caja-volver"><button data-volver-caja>← Volver al resumen de Caja</button></div><div class="caja-form-panel caja-vacio"><div class="caja-vacio-icono">⚠</div><h4>No hay caja abierta</h4><p>Apertura una caja primero.</p><button class="btn-primario" id="ir-apertura-nc">Ir a Apertura</button></div></div>';
  ...
  form.addEventListener('submit', function(e){
    ...
    var formNC = document.getElementById('form-nota-credito');
    formNC.insertAdjacentHTML('beforebegin','<div class="caja-confirmacion"><span>✓</span><div><strong>Nota de credito ' + nc.comprobante + ' emitida</strong><span>' + pago.paciente + ' · ' + pago.concepto + ' · ' + formatoMoneda(pago.monto) + ' · comprobante original ' + pago.comprobante + ' anulado</span></div></div>');
